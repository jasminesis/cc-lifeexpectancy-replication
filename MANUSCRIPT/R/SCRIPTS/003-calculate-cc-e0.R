...
# Loading in the UN life tabes. Selecting the Period, Age, mx, ax, and country-identifiers.
dat_UNlifetables <- read.xlsx("../MANUSCRIPT/R/DATA-RAW/WPP2017_MORT_F17_1_ABRIDGED_LIFE_TABLE_BOTH_SEXES.xlsx", 
                    sheet = 3,
                    startRow = 17) %>%
  ...
  left_join(., countrycodes) %>%
    ...
    mutate(Age = as.numeric(Age),
           ...
           ax = as.numeric(ax))
dat_UNlifetables[is.na(dat_UNlifetables)] <- 1

# The UN pop data is in 5-year intervals, but has a separate file that contains 0-1 and 1-4 age groups.
dat_UN0004 <- read.xlsx("../MANUSCRIPT/R/DATA-RAW/WPP2017_POP_F08_1_TOTAL_POPULATION_BY_BROAD_AGE_GROUP_BOTH_SEXES.xlsx",
                         sheet = 2,
                         startRow = 17) %>%
  filter(`Reference.date.(as.of.1.July)` == 2080) %>%
  left_join(., countrycodes) %>%
  dplyr::select(`0` = `0-1`,
                `1` = `0-4`,
                COUNTRY, CNTRY, ISO3) %>%
...
dat_UN05plus <- read.xlsx("../MANUSCRIPT/R/DATA-RAW/WPP2017_POP_F07_1_POPULATION_BY_AGE_BOTH_SEXES.xlsx",
                        sheet = 2,
                        startRow = 17) %>%
  filter(`Reference.date.(as.of.1.July)` == 2080) %>%
  left_join(., countrycodes) %>%
  ...
  dplyr::select(Period = `Reference.date.(as.of.1.July)`,
                Age, Pop,
                COUNTRY, CNTRY, ISO3) %>%
  separate(Age, c("Age", NA), sep = '-' ) %>%
  mutate(Age = if_else(Age == "100+", 95, as.numeric(Age))) %>%
...
dat_UN <- rbind(dat_UN0004, dat_UN05plus) %>%
  mutate(Pop = Pop * 1000,
